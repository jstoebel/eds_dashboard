class HelpsController < ApplicationController
  ...
  # simple framework for displaying help artilces for users
  ...
  def home
    ...
    contents = Dir[view_path] # get all help articles
    re = Regexp.new(base_path+"/_(?<filebase>.+?).html.md")
    @article_names = contents.map {|f| re.match(f)[:filebase] }
    @article_name = params[:article] # if an article was requested, grab it here
  end
