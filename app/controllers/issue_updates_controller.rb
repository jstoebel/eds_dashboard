...
class IssueUpdatesController < ApplicationController
  ...
  def create
    # saves a new update to the database, redirects to index
    # if successful, rerenders page if error.
    @issue = Issue.find(params[:issue_id])
    ...
    begin
      ...
      flash[:info] = 'New update added but there was a problem sending email alerts. ' \
  ...
  def index
    ...
    @updates = @issue
               .issue_updates
               .sorted.where(visible: true)
               .select { |r| can? :read, r } # no additional auth needed. If you can? the issue you can? the updates
  end
